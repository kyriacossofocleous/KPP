...
int InitSaveData()
{
  fpDat = fopen("strato_dp.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : strato_dp.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveError()
{
  fpErr = fopen("strato_dp_Err.dat", "w");
  if( fpErr == 0 ) {
    printf("\n Can't create file : strato_dp_Err.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveE()
{
  fpE = fopen("strato_dp_E.dat","w");
  if ( fpE == 0 ){
    printf("\n Can't create file: strato_dp_E.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("strato_dp.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : strato_dp.m");
    exit(1);
  }

  fprintf(fpMatlab, "load strato_dp.dat;\n");
  fprintf(fpMatlab, "%sc = strato_dp;\n", prefix);
  fprintf(fpMatlab, "clear strato_dp;\n");
