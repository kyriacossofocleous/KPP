...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   float Ynew[4], Fcn0[4], Fcn[4],
       dFdT[4],
       Jac0[15], Ghimj[15];
   float K[4 * ros_S];
   float H, T, Hnew, HC, HG, Fac, Tau;
   float Err, Yerr[4];
   int Pivot[4], Direction, ioffset, j, istage;
      ...
      *Hexit = H;
      ...
      { /* WHILE STEP NOT ACCEPTED */
         ...
         for (istage = 1; istage <= ros_S; istage++)
         {
            ...
            /* Current istage offset. Current istage vector is K[ioffset:ioffset+4-1] */
            ioffset = 4 * (istage - 1);

            /* For the 1st istage the function has been computed previously */
            if (istage == 1)
               WCOPY(4, Fcn0, 1, Fcn, 1);
            else
            { /* istage>1 and a new function evaluation is needed at current istage */
               if (ros_NewF[istage - 1])
               {
                  ...
                  for (j = 1; j <= istage - 1; j++)
                     WAXPY(4, ros_A[(istage - 1) * (istage - 2) / 2 + j - 1],
                           &K[4 * (j - 1)], 1, Ynew, 1);
         ...
         WCOPY(4, Y, 1, Ynew, 1);
         for (j = 1; j <= ros_S; j++)
            WAXPY(4, ros_M[j - 1], &K[4 * (j - 1)], 1, Ynew, 1);

         /*~~~>  Compute the error estimation   */
         WSCAL(4, ZERO, Yerr, 1);
         for (j = 1; j <= ros_S; j++)
            WAXPY(4, ros_E[j - 1], &K[4 * (j - 1)], 1, Yerr, 1);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   ...
   WAXPY(4, (-ONE), Fcn0, 1, dFdT, 1);
   WSCAL(4, (ONE / (float)Delta), dFdT, 1);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   ...
   { /* while Singular */
      ...
      WCOPY(15, Jac0, 1, Ghimj, 1);
      WSCAL(15, (-ONE), Ghimj, 1);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   ...
   /*~~~> Note: for a full matrix use Lapack:
      NRHS = 1
      DGETRS( 'N', 4 , NRHS, A, 4, Pivot, b, 4, INFO ) */
