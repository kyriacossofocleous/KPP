...
int InitSaveData()
{
  fpDat = fopen("saprc.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : saprc.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveError()
{
  fpErr = fopen("saprc_Err.dat", "w");
  if( fpErr == 0 ) {
    printf("\n Can't create file : saprc_Err.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveE()
{
  fpE = fopen("saprc_E.dat","w");
  if ( fpE == 0 ){
    printf("\n Can't create file: saprc_E.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("saprc.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : saprc.m");
    exit(1);
  }

  fprintf(fpMatlab, "load saprc.dat;\n");
  fprintf(fpMatlab, "%sc = saprc;\n", prefix);
  fprintf(fpMatlab, "clear saprc;\n");
