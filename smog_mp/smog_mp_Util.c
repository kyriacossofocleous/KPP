...
int InitSaveData()
{
  fpDat = fopen("smog_mp.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : smog_mp.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveError()
{
  fpErr = fopen("smog_mp_Err.dat", "w");
  if( fpErr == 0 ) {
    printf("\n Can't create file : smog_mp_Err.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveE()
{
  fpE = fopen("smog_mp_E.dat","w");
  if ( fpE == 0 ){
    printf("\n Can't create file: smog_mp_E.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("smog_mp.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : smog_mp.m");
    exit(1);
  }

  fprintf(fpMatlab, "load smog_mp.dat;\n");
  fprintf(fpMatlab, "%sc = smog_mp;\n", prefix);
  fprintf(fpMatlab, "clear smog_mp;\n");
