...
! 
! File                 : chapman_f90_mp_Hessian.f90
...
MODULE chapman_f90_mp_Hessian

  USE chapman_f90_mp_Parameters
  USE chapman_f90_mp_HessianSP
...
! D2A(1) = d^2 A(4) / dV(1)dV(2)
  D2A(1) = RCT(4)
! D2A(2) = d^2 A(5) / dV(2)dV(3)
  D2A(2) = RCT(5)
! D2A(3) = d^2 A(6) / dV(1)dV(4)
  D2A(3) = RCT(6)
...
! HESS(2) = d^2 Vdot(1)/{dV(1)dV(4)} = d^2 Vdot(1)/{dV(4)dV(1)}
  HESS(2) = -D2A(3)
! HESS(3) = d^2 Vdot(2)/{dV(1)dV(2)} = d^2 Vdot(2)/{dV(2)dV(1)}
  HESS(3) = -D2A(1)
! HESS(4) = d^2 Vdot(2)/{dV(2)dV(3)} = d^2 Vdot(2)/{dV(3)dV(2)}
  HESS(4) = -D2A(2)
! HESS(5) = d^2 Vdot(3)/{dV(1)dV(4)} = d^2 Vdot(3)/{dV(4)dV(1)}
  HESS(5) = D2A(3)
! HESS(6) = d^2 Vdot(3)/{dV(2)dV(3)} = d^2 Vdot(3)/{dV(3)dV(2)}
  HESS(6) = -D2A(2)
! HESS(7) = d^2 Vdot(4)/{dV(1)dV(4)} = d^2 Vdot(4)/{dV(4)dV(1)}
  HESS(7) = -D2A(3)
! HESS(8) = d^2 Vdot(4)/{dV(2)dV(3)} = d^2 Vdot(4)/{dV(3)dV(2)}
  HESS(8) = D2A(2)
...
! Compute the vector HTU =(Hess x U2)^T * U1 = d (Jac^T*U1)/d Var * U2
  HTU(1) = HESS(1)*(U1(1)*U2(2))+HESS(2)*(U1(1)*U2(4))+HESS(3)*(U1(2)*U2(2))+HESS(5)*(U1(3)*U2(4))+HESS(7)*(U1(4)*U2(4))
  HTU(2) = HESS(1)*(U1(1)*U2(1))+HESS(3)*(U1(2)*U2(1))+HESS(4)*(U1(2)*U2(3))+HESS(6)*(U1(3)*U2(3))+HESS(8)*(U1(4)*U2(3))
  HTU(3) = HESS(4)*(U1(2)*U2(2))+HESS(6)*(U1(3)*U2(2))+HESS(8)*(U1(4)*U2(2))
  HTU(4) = HESS(2)*(U1(1)*U2(1))+HESS(5)*(U1(3)*U2(1))+HESS(7)*(U1(4)*U2(1))
...
! Compute the vector HU =(Hess x U2) * U1 = d (Jac*U1)/d Var * U2
  HU(1) = HESS(1)*(U1(1)*U2(2))+HESS(1)*(U1(2)*U2(1))+HESS(2)*(U1(1)*U2(4))+HESS(2)*(U1(4)*U2(1))
  HU(2) = HESS(3)*(U1(1)*U2(2))+HESS(3)*(U1(2)*U2(1))+HESS(4)*(U1(2)*U2(3))+HESS(4)*(U1(3)*U2(2))
  HU(3) = HESS(5)*(U1(1)*U2(4))+HESS(5)*(U1(4)*U2(1))+HESS(6)*(U1(2)*U2(3))+HESS(6)*(U1(3)*U2(2))
  HU(4) = HESS(7)*(U1(1)*U2(4))+HESS(7)*(U1(4)*U2(1))+HESS(8)*(U1(2)*U2(3))+HESS(8)*(U1(3)*U2(2))
...
END MODULE chapman_f90_mp_Hessian
