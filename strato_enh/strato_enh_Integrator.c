...
int Rosenbrock(float Y[], float Tstart, float Tend,
               ...
               void (*ode_Fun)(double, float[], float[]),
...
int RosenbrockIntegrator(
   ...
   //  void (*ode_Fun2)(double, float[], float[]),
    ...
    float ros_Alpha[], float ros_Gamma[],
    float ros_ELO, char ros_NewF[],
    char Autonomous, char VectorTol, int Max_no_steps,
    float Roundoff, float Hmin, float Hmax, float Hstart,
    float FacMin, float FacMax, float FacRej, float FacSafe,
    float *Texit, float *Hexit);
char ros_PrepareMatrix(
    float *H,
    int Direction, float gam, float Jac0[],
    float Ghimj[], int Pivot[]);
float ros_ErrorNorm(
    float Y[], float Ynew[], float Yerr[],
    float AbsTol[], float RelTol[],
    char VectorTol);
int ros_ErrorMsg(int Code, float T, float H);
...
void FunTemplate(double T, float Y[], float Ydot[]);
...
void WAXPY(int N, float Alpha, float X[], int incX, float Y[], int incY);
void WSCAL(int N, float Alpha, float X[], int incX);
float WLAMCH(char C);
void Ros2(int *ros_S, float ros_A[], float ros_C[],
          float ros_M[], float ros_E[],
          float ros_Alpha[], float ros_Gamma[],
          char ros_NewF[], float *ros_ELO, char *ros_Name);
void Ros3(int *ros_S, float ros_A[], float ros_C[],
          float ros_M[], float ros_E[],
          float ros_Alpha[], float ros_Gamma[],
          char ros_NewF[], float *ros_ELO, char *ros_Name);
void Ros4(int *ros_S, float ros_A[], float ros_C[],
          float ros_M[], float ros_E[],
          float ros_Alpha[], float ros_Gamma[],
          char ros_NewF[], float *ros_ELO, char *ros_Name);
void Rodas3(int *ros_S, float ros_A[], float ros_C[],
            float ros_M[], float ros_E[],
            float ros_Alpha[], float ros_Gamma[],
            char ros_NewF[], float *ros_ELO, char *ros_Name);
void Rodas4(int *ros_S, float ros_A[], float ros_C[],
            float ros_M[], float ros_E[],
            float ros_Alpha[], float ros_Gamma[],
            char ros_NewF[], float *ros_ELO, char *ros_Name);
int KppDecomp(float A[]);
void KppSolve(float A[], float b[]);
void Update_SUN();
void Update_SUN_D();
void Update_RCONST();
int SaveData();
int SaveError(int s);
int SaveE(float E, int s);

/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void INTEGRATE(float TIN, float TOUT)
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   static float RPAR[20];
...
#define Smax 6
   int Method, ros_S;
   float ros_M[Smax], ros_E[Smax];
   float ros_A[Smax * (Smax - 1) / 2], ros_C[Smax * (Smax - 1) / 2];
   float ros_Alpha[Smax], ros_Gamma[Smax], ros_ELO;
   char ros_NewF[Smax], ros_Name[12];
   /*~~~>  Local variables    */
   int Max_no_steps, IERR, i, UplimTol;
   char Autonomous, VectorTol;
   float Roundoff, FacMin, FacMax, FacRej, FacSafe;
   float Hmin, Hmax, Hstart, Hexit, Texit;
   /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   float Ynew[34], Fcn0[34], Fcn[34],
       dFdT[34],
       Jac0[280], Ghimj[280];
   float K[34 * ros_S];
   float H, T, Hnew, HC, HG, Fac, Tau;
   float Err, Yerr[34];
   int Pivot[34], Direction, ioffset, j, istage;
      ...
      *Hexit = H;
      ...
      { /* WHILE STEP NOT ACCEPTED */
         ...
         for (istage = 1; istage <= ros_S; istage++)
         {
            ...
            /* Current istage offset. Current istage vector is K[ioffset:ioffset+34-1] */
            ioffset = 34 * (istage - 1);

            /* For the 1st istage the function has been computed previously */
            if (istage == 1)
               WCOPY(34, Fcn0, 1, Fcn, 1);
            else
            { /* istage>1 and a new function evaluation is needed at current istage */
               if (ros_NewF[istage - 1])
               {
                  ...
                  for (j = 1; j <= istage - 1; j++)
                     WAXPY(34, ros_A[(istage - 1) * (istage - 2) / 2 + j - 1],
                           &K[34 * (j - 1)], 1, Ynew, 1);
         ...
         WCOPY(34, Y, 1, Ynew, 1);
         for (j = 1; j <= ros_S; j++)
            WAXPY(34, ros_M[j - 1], &K[34 * (j - 1)], 1, Ynew, 1);

         /*~~~>  Compute the error estimation   */
         WSCAL(34, ZERO, Yerr, 1);
         for (j = 1; j <= ros_S; j++)
            WAXPY(34, ros_E[j - 1], &K[34 * (j - 1)], 1, Yerr, 1);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   ...
   WAXPY(34, (-ONE), Fcn0, 1, dFdT, 1);
   WSCAL(34, (ONE / (float)Delta), dFdT, 1);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   ...
   { /* while Singular */
      ...
      WCOPY(280, Jac0, 1, Ghimj, 1);
      WSCAL(280, (-ONE), Ghimj, 1);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   float g = 1.70710678118655; /* 1.0 + 1.0/SQRT(2.0) */
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   ...
   *ros_S = 3;
   ...
   /*~~~>     C_{i,j} = ros_C[ (i-1)*(i-2)/2 + j -1]  */
   ros_C[0] = (float)(-1.0156171083877702091975600115545);
   ros_C[1] = (float)4.0759956452537699824805835358067;
   ros_C[2] = (float)9.2076794298330791242156818474003;
   ...
   /*~~~> M_i = Coefficients for new step solution */
   ros_M[0] = (float)1.0;
   ros_M[1] = (float)6.1697947043828245592553615689730;
   ros_M[2] = (float)(-0.4277225654321857332623837380651);

   /*~~~> E_i = Coefficients for error estimator */
   ros_E[0] = (float)0.5;
   ros_E[1] = (float)(-2.9079558716805469821718236208017);
   ros_E[2] = (float)0.2235406989781156962736090927619;
...
!    main and the embedded scheme orders plus 1 */
   *ros_ELO = (float)3.0;
   ...
   /*~~~> Y_stage_i ~ Y( T + H*Alpha_i ) */
   ros_Alpha[0] = (float)0.0;
   ros_Alpha[1] = (float)0.43586652150845899941601945119356;
   ros_Alpha[2] = (float)0.43586652150845899941601945119356;

   /*~~~> Gamma_i = \sum_j  gamma_{i,j}  */
   ros_Gamma[0] = (float)0.43586652150845899941601945119356;
   ros_Gamma[1] = (float)0.24291996454816804366592249683314;
   ros_Gamma[2] = (float)2.1851380027664058511513169485832;
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   ...
   *ros_S = 4;
   ...
   /*~~~> The coefficient matrices A and C are strictly lower triangular.
    ...
    The general mapping formula is:
       A_{i,j} = ros_A[ (i-1)*(i-2)/2 + j -1 ]  */
   ros_A[0] = (float)0.2000000000000000e+01;
   ros_A[1] = (float)0.1867943637803922e+01;
   ros_A[2] = (float)0.2344449711399156;
   ros_A[3] = ros_A[1];
   ros_A[4] = ros_A[2];
   ros_A[5] = (float)0.0;

   /*~~~>     C(i,j) = (float)ros_C( (i-1)*(i-2)/2 + j )  */
   ros_C[0] = (float)(-0.7137615036412310e+01);
   ros_C[1] = (float)(0.2580708087951457e+01);
   ros_C[2] = (float)(0.6515950076447975);
   ros_C[3] = (float)(-0.2137148994382534e+01);
   ros_C[4] = (float)(-0.3214669691237626);
   ros_C[5] = (float)(-0.6949742501781779);
   ...
   /*~~~> M_i = Coefficients for new step solution */
   ros_M[0] = (float)0.2255570073418735e+01;
   ros_M[1] = (float)0.2870493262186792;
   ros_M[2] = (float)0.4353179431840180;
   ros_M[3] = (float)0.1093502252409163e+01;

   /*~~~> E_i  = Coefficients for error estimator */
   ros_E[0] = (float)(-0.2815431932141155);
   ros_E[1] = (float)(-0.7276199124938920e-01);
   ros_E[2] = (float)(-0.1082196201495311);
   ros_E[3] = (float)(-0.1093502252409163e+01);
...
!    main and the embedded scheme orders plus 1 */
   *ros_ELO = (float)4.0;
   ...
   /*~~~> Y_stage_i ~ Y( T + H*Alpha_i ) */
   ros_Alpha[0] = (float)0.0;
   ros_Alpha[1] = (float)0.1145640000000000e+01;
   ros_Alpha[2] = (float)0.6552168638155900;
   ros_Alpha[3] = (float)ros_Alpha[2];

   /*~~~> Gamma_i = \sum_j  gamma_{i,j}  */
   ros_Gamma[0] = (float)(0.5728200000000000);
   ros_Gamma[1] = (float)(-0.1769193891319233e+01);
   ros_Gamma[2] = (float)(0.7592633437920482);
   ros_Gamma[3] = (float)(-0.1049021087100450);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   ...
   *ros_S = 4;
   ...
   ros_C[3] = (float)1.0;
   ros_C[4] = (float)(-1.0);
   ros_C[5] = (float)(-2.66666666666667); /* -8/3 */
...
!    main and the embedded scheme orders plus 1 */
   *ros_ELO = (float)3.0;
   ...
   /*~~~> Y_stage_i ~ Y( T + H*Alpha_i ) */
   ros_Alpha[0] = (float)0.0;
   ros_Alpha[1] = (float)0.0;
   ros_Alpha[2] = (float)1.0;
   ros_Alpha[3] = (float)1.0;

   /*~~~> Gamma_i = \sum_j  gamma_{i,j}  */
   ros_Gamma[0] = (float)0.5;
   ros_Gamma[1] = (float)1.5;
   ros_Gamma[2] = (float)0.0;
   ros_Gamma[3] = (float)0.0;
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   ...
   *ros_S = 6;
   ...
   /*~~~> Y_stage_i ~ Y( T + H*Alpha_i ) */
   ros_Alpha[0] = (float)0.000;
   ros_Alpha[1] = (float)0.386;
   ros_Alpha[2] = (float)0.210;
   ros_Alpha[3] = (float)0.630;
   ros_Alpha[4] = (float)1.000;
   ros_Alpha[5] = (float)1.000;

   /*~~~> Gamma_i = \sum_j  gamma_{i,j}  */
   ros_Gamma[0] = (float)0.2500000000000000;
   ros_Gamma[1] = (float)(-0.1043000000000000);
   ros_Gamma[2] = (float)0.1035000000000000;
   ros_Gamma[3] = (float)(-0.3620000000000023e-01);
   ros_Gamma[4] = (float)0.0;
   ros_Gamma[5] = (float)0.0;

   /*~~~> The coefficient matrices A and C are strictly lower triangular.
    The lower triangular (subdiagonal) elements are stored in row-wise order:
    A(2,1) = ros_A[0], A(3,1)=ros_A[1], A(3,2)=ros_A[2], etc.
    The general mapping formula is:  A_{i,j} = ros_A[ (i-1)*(i-2)/2 + j -1 ]  */
   ros_A[0] = (float)0.1544000000000000e+01;
   ros_A[1] = (float)0.9466785280815826;
   ros_A[2] = (float)0.2557011698983284;
   ros_A[3] = (float)0.3314825187068521e+01;
   ros_A[4] = (float)0.2896124015972201e+01;
   ros_A[5] = (float)0.9986419139977817;
   ros_A[6] = (float)0.1221224509226641e+01;
   ros_A[7] = (float)0.6019134481288629e+01;
   ros_A[8] = (float)0.1253708332932087e+02;
   ros_A[9] = (float)(-0.6878860361058950);
   ros_A[10] = ros_A[6];
   ros_A[11] = ros_A[7];
   ros_A[12] = ros_A[8];
   ros_A[13] = ros_A[9];
   ...
   /*~~~>     C_{i,j} = ros_C[ (i-1)*(i-2)/2 + j -1]  */
   ros_C[0] = (float)(-0.5668800000000000e+01);
   ros_C[1] = (float)(-0.2430093356833875e+01);
   ros_C[2] = (float)(-0.2063599157091915);
   ros_C[3] = (float)(-0.1073529058151375);
   ros_C[4] = (float)(-0.9594562251023355e+01);
   ros_C[5] = (float)(-0.2047028614809616e+02);
   ros_C[6] = (float)(0.7496443313967647e+01);
   ros_C[7] = (float)(-0.1024680431464352e+02);
   ros_C[8] = (float)(-0.3399990352819905e+02);
   ros_C[9] = (float)(0.1170890893206160e+02);
   ros_C[10] = (float)(0.8083246795921522e+01);
   ros_C[11] = (float)(-0.7981132988064893e+01);
   ros_C[12] = (float)(-0.3152159432874371e+02);
   ros_C[13] = (float)(0.1631930543123136e+02);
   ros_C[14] = (float)(-0.6058818238834054e+01);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
   ...
   /*~~~> Note: for a full matrix use Lapack:
      NRHS = 1
      DGETRS( 'N', 34 , NRHS, A, 34, Pivot, b, 34, INFO ) */
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void JacTemplate(float T, float Y[], float Jcb[])
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   
