...
int InitSaveData()
{
  fpDat = fopen("strato_mp.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : strato_mp.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveError()
{
  fpErr = fopen("strato_mp_Err.dat", "w");
  if( fpErr == 0 ) {
    printf("\n Can't create file : strato_mp_Err.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveE()
{
  fpE = fopen("strato_mp_E.dat","w");
  if ( fpE == 0 ){
    printf("\n Can't create file: strato_mp_E.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("strato_mp.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : strato_mp.m");
    exit(1);
  }

  fprintf(fpMatlab, "load strato_mp.dat;\n");
  fprintf(fpMatlab, "%sc = strato_mp;\n", prefix);
  fprintf(fpMatlab, "clear strato_mp;\n");
