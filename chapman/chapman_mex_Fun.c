...
/* Check for the right number and size of input arguments */
 if ( nrhs != 3 ) {
   mexErrMsgTxt("chapman_Fun requires 3 input vectors: V(5), F(2), RCT(10)");
 }
 mrows =  mxGetM(prhs[0]); mcols = mxGetN(prhs[0]);
 if ( ( mrows != 5 )||( mcols != 1 ) ) {
   mexPrintf("First chapman_Fun input argument is of size V(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("First chapman_Fun input argument should be a column vector V(5,1)");
 }
 mrows =  mxGetM(prhs[1]); mcols = mxGetN(prhs[1]);
 if ( ( mrows != 2 )||( mcols != 1 ) ) {
   mexPrintf("Second chapman_Fun input argument is of size F(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("Second chapman_Fun input argument should be a column vector F(2,1)");
 }
 mrows =  mxGetM(prhs[2]); mcols = mxGetN(prhs[2]);
 if ( (  mrows != 10 )||( mcols != 1 ) ) {
   mexPrintf("Third chapman_Fun input argument is of size RCT(%d,%d).",  
               mrows, mcols);
   mexErrMsgTxt("Third chapman_Fun input argument should be a column vector RCT(10,1)");
 }
 
/* Check for the right number of output arguments */
 if ( nlhs != 1 ) {
   mexErrMsgTxt("chapman_Fun requires 1 output column vector: Vdot(5)");
 }
