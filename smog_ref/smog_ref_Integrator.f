...
C ***********************************************************
C
      REAL*8 FUNCTION CONTR5(I,X,CONT,LRC)
C ----------------------------------------------------------
...
C-----------------------------------------------------------------------
C  SOLUTION OF LINEAR SYSTEM, A*X = B ,
C                  VERSION BANDED AND COMPLEX-REAL*8.
C  INPUT..
C    N      ORDER OF MATRIX A.
C    NDIM   DECLARED DIMENSION OF ARRAY  A .
...
C --- CONTINUOUS OUTPUT FOR RADAU5
 ...
 99     FORMAT(1X,'X =',F5.2,'    Y =',2E18.10,'    NSTEP =',I4)
	...
	IMPLICIT NONE
        INCLUDE 'smog_ref_Parameters.h'
        INCLUDE 'smog_ref_Global.h'
        REAL*8 V(NVAR), FCT(NVAR)
	REAL*8 T, TOLD, RPAR(1)
	INTEGER N, IPAR(1)
        TOLD = TIME
        TIME = T
        CALL Update_SUN()
