...
int InitSaveData()
{
  fpDat = fopen("tropo_dp.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : tropo_dp.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveError()
{
  fpErr = fopen("tropo_dp_Err.dat", "w");
  if( fpErr == 0 ) {
    printf("\n Can't create file : tropo_dp_Err.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveE()
{
  fpE = fopen("tropo_dp_E.dat","w");
  if ( fpE == 0 ){
    printf("\n Can't create file: tropo_dp_E.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("tropo_dp.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : tropo_dp.m");
    exit(1);
  }

  fprintf(fpMatlab, "load tropo_dp.dat;\n");
  fprintf(fpMatlab, "%sc = tropo_dp;\n", prefix);
  fprintf(fpMatlab, "clear tropo_dp;\n");
