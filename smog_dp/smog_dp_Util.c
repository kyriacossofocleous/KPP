...
int InitSaveData()
{
  fpDat = fopen("smog_dp.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : smog_dp.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveError()
{
  fpErr = fopen("smog_dp_Err.dat", "w");
  if( fpErr == 0 ) {
    printf("\n Can't create file : smog_dp_Err.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveE()
{
  fpE = fopen("smog_dp_E.dat","w");
  if ( fpE == 0 ){
    printf("\n Can't create file: smog_dp_E.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("smog_dp.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : smog_dp.m");
    exit(1);
  }

  fprintf(fpMatlab, "load smog_dp.dat;\n");
  fprintf(fpMatlab, "%sc = smog_dp;\n", prefix);
  fprintf(fpMatlab, "clear smog_dp;\n");
