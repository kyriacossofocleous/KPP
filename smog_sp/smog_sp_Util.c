...
int InitSaveData()
{
  fpDat = fopen("smog_sp.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : smog_sp.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveError()
{
  fpErr = fopen("smog_sp_Err.dat", "w");
  if( fpErr == 0 ) {
    printf("\n Can't create file : smog_sp_Err.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveE()
{
  fpE = fopen("smog_sp_E.dat","w");
  if ( fpE == 0 ){
    printf("\n Can't create file: smog_sp_E.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("smog_sp.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : smog_sp.m");
    exit(1);
  }

  fprintf(fpMatlab, "load smog_sp.dat;\n");
  fprintf(fpMatlab, "%sc = smog_sp;\n", prefix);
  fprintf(fpMatlab, "clear smog_sp;\n");
