/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
/*                                                                  */
/* Auxiliary Routines File                                          */
/*                                                                  */
/* Generated by KPP-2.2.3 symbolic chemistry Kinetics PreProcessor  */
/*       (http://www.cs.vt.edu/~asandu/Software/KPP)                */
/* KPP is distributed under GPL, the general public licence         */
/*       (http://www.gnu.org/copyleft/gpl.html)                     */
/* (C) 1995-1997, V. Damian & A. Sandu, CGRER, Univ. Iowa           */
/* (C) 1997-2005, A. Sandu, Michigan Tech, Virginia Tech            */
/*     With important contributions from:                           */
/*        M. Damian, Villanova University, USA                      */
/*        R. Sander, Max-Planck Institute for Chemistry, Mainz, Germany */
/*                                                                  */
/* File                 : tropo_Util.c                              */
/* Time                 : Thu Jan 27 12:49:23 2022                  */
/* Working directory    : /home/kyriacos/CyprusInstitute/kpp/tropo  */
/* Equation file        : tropo.kpp                                 */
/* Output root filename : tropo                                     */
/*                                                                  */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */

#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <string.h>
#include <sys/time.h>
#include <float.h>
#include "tropo_Parameters.h"
#include "tropo_Global.h"
#include "tropo_Sparse.h"



/* User INLINED Utility Functions                                   */

/* End INLINED Utility Functions                                    */

/* Utility Functions from KPP_HOME/util/util                        */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
/*                                                                  */
/* UTIL - Utility functions                                         */
/*   Arguments :                                                    */
/*                                                                  */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */

/*
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
*/
double min( double x, double y )
{
  return ( x <= y ) ? x : y;
}

double max( double x, double y )
{
  return ( x >= y ) ? x : y;
}

double
Stopwatch(double t0)
{
  struct timeval t;
  gettimeofday(&t, NULL);
  return (double)t.tv_sec + (double)t.tv_usec / 1e6 - t0;
}

static FILE *fpDat = 0;
static FILE *fpErr = 0;
static FILE *fpE = 0;

int InitSaveData()
{
  fpDat = fopen("tropo.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : tropo.dat");
    exit(1);
  }
  return 0;
}

int InitSaveError()
{
  fpErr = fopen("tropo_Err.dat", "w");
  if( fpErr == 0 ) {
    printf("\n Can't create file : tropo_Err.dat");
    exit(1);
  }
  return 0;
}

int InitSaveE()
{
  fpE = fopen("tropo_E.dat","w");
  if ( fpE == 0 ){
    printf("\n Can't create file: tropo_E.dat");
    exit(1);
  }
  return 0;
}

int SaveE(double Err, int s)
{
  fprintf( fpE, "%6.3f ", TIME/3600.0);
  fprintf( fpE, "%e ", Err);
  fprintf( fpE, "%e ", SUN);
  fprintf( fpE, "%d ", s);
  fprintf( fpE, "\n");
  return 0;
}


int SaveData()
{
int i;

  fprintf( fpDat, "%6.1f ", TIME/3600.0 );
  for( i = 0; i < NLOOKAT; i++ )
    fprintf( fpDat, "%24.16e ", C[ LOOKAT[i] ]/CFACTOR );
  // fprintf( fpDat, "%d ", s);
  fprintf( fpDat, "\n");
  return 0;
}

int SaveError(int s)
{
int i;

  fprintf( fpErr, "%6.1f ", TIME/3600.0 );
  for( i = 0; i < NLOOKAT; i++ )
    fprintf( fpErr, "%24.16e ", E[ LOOKAT[i] ] );
  fprintf( fpErr, "%d ", s);
  fprintf( fpErr, "\n");
  return 0;
}

int CloseSaveData()
{
  fclose( fpDat );
  return 0;
}

int CloseSaveError()
{
  fclose( fpErr );
  return 0;
}

int CloseSaveE()
{
  fclose( fpE );
  return 0;
}

int GenerateMatlab( char * prefix )
{
int i;
FILE *fpMatlab;
  
  fpMatlab = fopen("tropo.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : tropo.m");
    exit(1);
  }

  fprintf(fpMatlab, "load tropo.dat;\n");
  fprintf(fpMatlab, "%sc = tropo;\n", prefix);
  fprintf(fpMatlab, "clear tropo;\n");
  fprintf(fpMatlab, "%st=%sc(:,1);\n", prefix, prefix);
  fprintf(fpMatlab, "%sc(:,1)=[];\n", prefix);
  
  for( i = 0; i < NLOOKAT; i++ )
    fprintf( fpMatlab, "%s%s = %sc(:,%d);\n", 
            prefix, SPC_NAMES[LOOKAT[i]], 
            prefix, i+1 );
  
  fclose( fpMatlab );
  return 0;
}
/* End Utility Functions from KPP_HOME/util/util                    */
/* End of UTIL function                                             */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */


/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
/*                                                                  */
/* Shuffle_user2kpp - function to copy concentrations from USER to KPP */
/*   Arguments :                                                    */
/*      V_USER    - Concentration of variable species in USER's order */
/*      V         - Concentrations of variable species (local)      */
/*                                                                  */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */

void Shuffle_user2kpp( 
  double V_USER[],                       /* Concentration of variable species in USER's order */
  double V[]                             /* Concentrations of variable species (local) */
)
{
  V[80] = V_USER[0];
  V[75] = V_USER[1];
  V[56] = V_USER[2];
  V[77] = V_USER[4];
  V[0] = V_USER[5];
  V[73] = V_USER[6];
  V[28] = V_USER[7];
  V[18] = V_USER[8];
  V[19] = V_USER[9];
  V[29] = V_USER[10];
  V[39] = V_USER[11];
  V[59] = V_USER[12];
  V[23] = V_USER[13];
  V[32] = V_USER[14];
  V[1] = V_USER[15];
  V[76] = V_USER[16];
  V[78] = V_USER[17];
  V[22] = V_USER[18];
  V[70] = V_USER[19];
  V[26] = V_USER[20];
  V[31] = V_USER[21];
  V[24] = V_USER[22];
  V[61] = V_USER[23];
  V[35] = V_USER[24];
  V[33] = V_USER[25];
  V[34] = V_USER[26];
  V[41] = V_USER[27];
  V[2] = V_USER[30];
  V[37] = V_USER[31];
  V[27] = V_USER[32];
  V[25] = V_USER[33];
  V[30] = V_USER[34];
  V[82] = V_USER[35];
  V[63] = V_USER[36];
  V[74] = V_USER[37];
  V[72] = V_USER[38];
  V[44] = V_USER[39];
  V[57] = V_USER[40];
  V[20] = V_USER[41];
  V[21] = V_USER[42];
  V[54] = V_USER[43];
  V[42] = V_USER[44];
  V[49] = V_USER[45];
  V[15] = V_USER[46];
  V[16] = V_USER[47];
  V[10] = V_USER[48];
  V[11] = V_USER[49];
  V[12] = V_USER[50];
  V[13] = V_USER[51];
  V[14] = V_USER[52];
  V[3] = V_USER[53];
  V[4] = V_USER[54];
  V[5] = V_USER[55];
  V[6] = V_USER[56];
  V[7] = V_USER[57];
  V[8] = V_USER[58];
  V[17] = V_USER[59];
  V[9] = V_USER[60];
  V[40] = V_USER[61];
  V[83] = V_USER[62];
  V[81] = V_USER[63];
  V[36] = V_USER[64];
  V[71] = V_USER[65];
  V[64] = V_USER[66];
  V[50] = V_USER[67];
  V[62] = V_USER[68];
  V[45] = V_USER[69];
  V[60] = V_USER[70];
  V[43] = V_USER[71];
  V[53] = V_USER[72];
  V[38] = V_USER[73];
  V[46] = V_USER[74];
  V[47] = V_USER[75];
  V[79] = V_USER[76];
  V[66] = V_USER[77];
  V[51] = V_USER[78];
  V[52] = V_USER[79];
  V[58] = V_USER[80];
  V[65] = V_USER[81];
  V[48] = V_USER[82];
  V[55] = V_USER[83];
}

/* End of Shuffle_user2kpp function                                 */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */


/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
/*                                                                  */
/* Shuffle_kpp2user - function to restore concentrations from KPP to USER */
/*   Arguments :                                                    */
/*      V         - Concentrations of variable species (local)      */
/*      V_USER    - Concentration of variable species in USER's order */
/*                                                                  */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */

void Shuffle_kpp2user( 
  double V[],                            /* Concentrations of variable species (local) */
  double V_USER[]                        /* Concentration of variable species in USER's order */
)
{
  V_USER[0] = V[80];
  V_USER[1] = V[75];
  V_USER[2] = V[56];
  V_USER[4] = V[77];
  V_USER[5] = V[0];
  V_USER[6] = V[73];
  V_USER[7] = V[28];
  V_USER[8] = V[18];
  V_USER[9] = V[19];
  V_USER[10] = V[29];
  V_USER[11] = V[39];
  V_USER[12] = V[59];
  V_USER[13] = V[23];
  V_USER[14] = V[32];
  V_USER[15] = V[1];
  V_USER[16] = V[76];
  V_USER[17] = V[78];
  V_USER[18] = V[22];
  V_USER[19] = V[70];
  V_USER[20] = V[26];
  V_USER[21] = V[31];
  V_USER[22] = V[24];
  V_USER[23] = V[61];
  V_USER[24] = V[35];
  V_USER[25] = V[33];
  V_USER[26] = V[34];
  V_USER[27] = V[41];
  V_USER[30] = V[2];
  V_USER[31] = V[37];
  V_USER[32] = V[27];
  V_USER[33] = V[25];
  V_USER[34] = V[30];
  V_USER[35] = V[82];
  V_USER[36] = V[63];
  V_USER[37] = V[74];
  V_USER[38] = V[72];
  V_USER[39] = V[44];
  V_USER[40] = V[57];
  V_USER[41] = V[20];
  V_USER[42] = V[21];
  V_USER[43] = V[54];
  V_USER[44] = V[42];
  V_USER[45] = V[49];
  V_USER[46] = V[15];
  V_USER[47] = V[16];
  V_USER[48] = V[10];
  V_USER[49] = V[11];
  V_USER[50] = V[12];
  V_USER[51] = V[13];
  V_USER[52] = V[14];
  V_USER[53] = V[3];
  V_USER[54] = V[4];
  V_USER[55] = V[5];
  V_USER[56] = V[6];
  V_USER[57] = V[7];
  V_USER[58] = V[8];
  V_USER[59] = V[17];
  V_USER[60] = V[9];
  V_USER[61] = V[40];
  V_USER[62] = V[83];
  V_USER[63] = V[81];
  V_USER[64] = V[36];
  V_USER[65] = V[71];
  V_USER[66] = V[64];
  V_USER[67] = V[50];
  V_USER[68] = V[62];
  V_USER[69] = V[45];
  V_USER[70] = V[60];
  V_USER[71] = V[43];
  V_USER[72] = V[53];
  V_USER[73] = V[38];
  V_USER[74] = V[46];
  V_USER[75] = V[47];
  V_USER[76] = V[79];
  V_USER[77] = V[66];
  V_USER[78] = V[51];
  V_USER[79] = V[52];
  V_USER[80] = V[58];
  V_USER[81] = V[65];
  V_USER[82] = V[48];
  V_USER[83] = V[55];
}

/* End of Shuffle_kpp2user function                                 */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */


/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
/*                                                                  */
/* GetMass - compute total mass of selected atoms                   */
/*   Arguments :                                                    */
/*      CL        - Concentration of all species (local)            */
/*      Mass      - value of mass balance                           */
/*                                                                  */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */

void GetMass( 
  double CL[],                           /* Concentration of all species (local) */
  double Mass[]                          /* value of mass balance */
)
{
}

/* End of GetMass function                                          */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */


