...
int InitSaveData()
{
  fpDat = fopen("saprc_dp.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : saprc_dp.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveError()
{
  fpErr = fopen("saprc_dp_Err.dat", "w");
  if( fpErr == 0 ) {
    printf("\n Can't create file : saprc_dp_Err.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveE()
{
  fpE = fopen("saprc_dp_E.dat","w");
  if ( fpE == 0 ){
    printf("\n Can't create file: saprc_dp_E.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("saprc_dp.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : saprc_dp.m");
    exit(1);
  }

  fprintf(fpMatlab, "load saprc_dp.dat;\n");
  fprintf(fpMatlab, "%sc = saprc_dp;\n", prefix);
  fprintf(fpMatlab, "clear saprc_dp;\n");
