...
int InitSaveData()
{
  fpDat = fopen("strato_sp.dat", "w");
  if( fpDat == 0 ) {
    printf("\n Can't create file : strato_sp.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveError()
{
  fpErr = fopen("strato_sp_Err.dat", "w");
  if( fpErr == 0 ) {
    printf("\n Can't create file : strato_sp_Err.dat");
    exit(1);
  }
  return 0;
}
...
int InitSaveE()
{
  fpE = fopen("strato_sp_E.dat","w");
  if ( fpE == 0 ){
    printf("\n Can't create file: strato_sp_E.dat");
    exit(1);
  }
  return 0;
}
...
FILE *fpMatlab;
  ...
  fpMatlab = fopen("strato_sp.m", "w");
  if( fpMatlab == 0 ) {
    printf("\n Can't create file : strato_sp.m");
    exit(1);
  }

  fprintf(fpMatlab, "load strato_sp.dat;\n");
  fprintf(fpMatlab, "%sc = strato_sp;\n", prefix);
  fprintf(fpMatlab, "clear strato_sp;\n");
