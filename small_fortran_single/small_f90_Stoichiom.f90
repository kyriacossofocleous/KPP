...
MODULE small_f90_Stoichiom

  USE small_f90_Parameters
  USE small_f90_StoichiomSP
...
!         This matrix is stored in a column-wise linearized format
      REAL(kind=sp) DFDR(NVAR*NCOEFF)

! Local vector with reactant products
      REAL(kind=sp) A_RPROD(NREACT)
      REAL(kind=sp) aj
      INTEGER i,j,k
...
!                     for each 1 <= j <= NCOEFF
!         This matrix is stored in a column-wise linearized format
      REAL(kind=sp) DJDR(NVAR*NCOEFF)

! Local vector for Jacobian of reactant products
      REAL(kind=sp) JV_RPROD(NJVRP)
      REAL(kind=sp) aj
      INTEGER i,j,k
...
END MODULE small_f90_Stoichiom
